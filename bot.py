...
def fetch_open_orders_map(session: httpx.Client) -> dict:
    # Pull more than default 50 to avoid missing existing trailing stops, which caused 403 "held_for_orders"
    try:
    ...
    for p in positions:
        ...
        if sym and qty and not has_sell.get(sym):
            try:
                ...
                # Swallow "insufficient qty / held_for_orders" noise
                msg = str(e)
...
async def main():
    print("=== Grandmaster Finnhub + Alpaca — PATCH v10 ===")
    alp = httpx.Client()
    ...
    while True:
        ...
        try:
            ...
            print(f"[ACCOUNT] equity={fmt_money(equity)}  cash={fmt_money(cash)}  bp={fmt_money(bp)}  dayPnL={fmt_money(day_pnl)}")
        except Exception as e:
        ...
        # Ensure trailing stops (with 500-limit open orders)
